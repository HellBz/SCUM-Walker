  ...
  data.routes.forEach(route => {
    ...
    div.innerHTML = `
      ...
      <span class="route-actions">
        ...
        <button class="route-icon ${visible ? '' : 'hidden'}" data-action="toggle-visibility" data-id="${route.id}" title="${visible ? I18n.t('routes.actions.hide', 'Auf Karte ausblenden') : I18n.t('routes.actions.show', 'Auf Karte einblenden')}">${visible ? '<svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M2 12s3-7 10-7 10 7 10 7-3 7-10 7-10-7-10-7Z"/><circle cx="12" cy="12" r="3"/></svg>' : '<svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M9.88 9.88a3 3 0 1 0 4.24 4.24"/><path d="M10.73 5.08A10.43 10.43 0 0 1 12 5c7 0 10 7 10 7a13.16 13.16 0 0 1-1.67 2.68"/><path d="M6.61 6.61A13.526 13.526 0 0 0 2 12s3 7 10 7a9.74 9.74 0 0 0 5.39-1.61"/><line x1="2" y1="2" x2="22" y2="22"/></svg>'}</button>
        <button class="route-icon" data-action="rename" data-id="${route.id}" title="${I18n.t('routes.actions.rename', 'Umbenennen')}"><svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4 1 1-4Z"/></svg></button>
        <button class="route-icon" data-action="delete" data-id="${route.id}" title="${I18n.t('routes.actions.delete', 'Löschen')}"><svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg></button>
      </span>
      <span class="route-count">${I18n.t('routes.points', '{{count}} Punkte').replace('{{count}}', route.records.length)}</span>
    `;
  ...
  filtered.forEach(poi => {
    ...
    div.innerHTML = `<span class="poi-label-span" data-id="${poi.id}" title="Auf POI zentrieren"><span class="poi-color" style="background:${poi.color}"></span>${escapeHtml(poi.label)}</span>
                     <span class="poi-actions">
                       ...
                       <button class="poi-more-btn" data-id="${poi.id}" title="${I18n.t('poi.actions.more', 'Mehr Aktionen')}"><svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="5" r="1"/><circle cx="12" cy="12" r="1"/><circle cx="12" cy="19" r="1"/></svg></button>
                       <div class="poi-dropdown" data-id="${poi.id}">
                         <button class="poi-edit" data-id="${poi.id}" title="${I18n.t('poi.actions.edit_title', 'POI bearbeiten')}"><svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4 1 1-4Z"/></svg> ${I18n.t('poi.actions.edit', 'Bearbeiten')}</button>
                         <button class="poi-image-btn" data-id="${poi.id}" title="${hasImage ? I18n.t('poi.actions.image_show', 'Bild anzeigen') : I18n.t('poi.actions.screenshot_title', 'Screenshot aus SCUM')}">${hasImage ? '<svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M2 12s3-7 10-7 10 7 10 7-3 7-10 7-10-7-10-7Z"/><circle cx="12" cy="12" r="3"/></svg>' : '<svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M23 19a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h4l2-3h6l2 3h4a2 2 0 0 1 2 2z"/><circle cx="12" cy="13" r="4"/></svg>'} ${hasImage ? I18n.t('poi.actions.image', 'Bild') : I18n.t('poi.actions.screenshot', 'Screenshot')}</button>
                         <button class="poi-upload-btn" data-id="${poi.id}" title="${I18n.t('poi.actions.upload_title', 'Bild hochladen')}"><svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg> ${I18n.t('poi.actions.upload', 'Upload')}</button>
                         <button class="poi-copy" data-id="${poi.id}" title="${I18n.t('poi.actions.copy_title', 'POI in Zwischenablage kopieren')}"><svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="9" y="9" width="10" height="10" rx="1.5" ry="1.5"/><path d="M7 17V7c0-1.1.9-2 2-2h7"/></svg> ${I18n.t('poi.actions.copy', 'Kopieren')}</button>
                         <button class="poi-delete" data-id="${poi.id}" title="${I18n.t('poi.actions.delete_title', 'POI löschen')}"><svg viewBox="0 0 24 24" width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg> ${I18n.t('poi.actions.delete', 'Löschen')}</button>
                       </div>
                     </span>`;
